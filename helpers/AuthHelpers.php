...
class AuthHelpers
{
            ...
            'rules' => [
                [
                    ...
                    'matchCallback' => function ($rule, $action) {
                        ...
                        if ($iduser == 'admin') { //loss untuk user lmt001 bagian permission
                            return TRUE;
                        }
    ...
    public static function FilterSidebar($items, $allowedRoutes)
    {
        ...
        foreach ($items as $i => $item) {
            if (isset($item['items']) && is_array($item['items'])) {
                $item['items'] = self::FilterSidebar($item['items'], $allowedRoutes);
                if (!empty($item['items'])) {
                    $allowedItems[] = $item;
                }
            } else {
                if ($userId == 'admin' || isset($item['visible'])) {
                    // Jika pengguna adalah 'LMT001' atau item tersebut sudah terlihat, tambahkan item ke daftar item yang diperbolehkan
                    $allowedItems[] = $item;
                } else {
                    $route = isset($item['url'][0]) ? $item['url'][0] : null;
                    if ($route !== null && isset($allowedRoutes[$route])) {
                        // Jika pengguna memiliki izin untuk route ini, tambahkan item ke daftar item yang diperbolehkan
                        $allowedItems[] = $item;
                    }
                }
            }
